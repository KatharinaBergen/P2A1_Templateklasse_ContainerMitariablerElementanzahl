...
public: //inline definition is necessary for templates. 
	...
	//Standardconstructor
	MyVector() : size{ 0 }, space{ 0 }, elem{ nullptr }
	{
		cout << "\nStandardconstructor called.\n";
	}

	//typical vector constructor, defines how many elements are held in the vector
	explicit MyVector(int s, T = T{}): size{ s }, space{ s }, elem{ new T[s]{} }
	{
		cout << "\nConstructor called." << " elem: " << elem << " elem input: " << s << std::endl;
	}

	//constructor for testing purposes
	MyVector(int size, int space, T content[]) : size{ size }, space{ space }, elem{ new T[size] {} }
	{
	...
	//Copyconstructor
	MyVector(const MyVector& v): size{v.size}, space {v.space}, elem{}
	{
	...
	T& at(int n) // Zugriffsbereich ueberprueft
	{
		if (n < 0 || size <= n)
		{
